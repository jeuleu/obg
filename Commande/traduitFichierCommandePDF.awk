...
input != FILENAME {
	...
	# nom du fichier
	fileBasename = getBaseName(FILENAME)
	fileType = getFileType(fileBasename)
	printf("Fichier '%s' - '%s' - '%s'", FILENAME, fileBasename, fileType)
...
/\/OBAG\/[0-9]*/ {
	...
	# mise a jour de la liste des documents de référence
	listeReferenceDocument = ""
	for (refDocument in tabRefDocument) {
		if (listeReferenceDocument != "") {
			listeReferenceDocument = listeReferenceDocument " | " 
		}
		listeReferenceDocument = listeReferenceDocument $0
	}
}
...
#	print "Ref facture : '" $0 "'"
	...
	# format référence : 2019 50 279 04/02/2019
	dateFacture = refFacture
	gsub(/.* /, "", dateFacture)	
}
...
function getFileType(fileBasename) {
	# On garde les majuscules du début
	fileType = fileBasename
	sub("[^A-Z].*$", "", fileType)
...
function adapteLignePourLaTaille(ligne) {
	...
	if ( attributs[4] != "UNICA" && attributs[4] != "") {
...
function traitementDeFinDeFichier() {	
	...
	for (i in tabLignes) {
		...
		ecritDansFichier(sprintf("'%03d;%s;%s;%s;%s;%s;%s;%s", i, ligne, fileBasename, fileType, dateFacture, totalFacture, totalProduits, listeReferenceDocument))
		delete tabLignes[i]
	}
